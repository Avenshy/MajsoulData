...
def set_max_data():
    max_data = {}
    with open(
        "./data/tables/item_definition/character.json", "r", encoding="utf-8"
    ) as f:
    ...
    with open("./data/tables/item_definition/item.json", "r", encoding="utf-8") as f:
        max_data["item"] = [
            item["id"] for item in json.load(f) if item["category"] == 5
        ]
    with open("./data/tables/item_definition/item.json", "r", encoding="utf-8") as f:
        max_data["loading_image"] = [
            loading_image["id"]
            for loading_image in json.load(f)
            if loading_image["category"] == 8
        ]
    with open(
        "./data/tables/item_definition/loading_image.json", "r", encoding="utf-8"
    ) as f:
        max_data["loading_image"] = list(
            set(
                max_data["loading_image"]
                + [loading_image["id"] for loading_image in json.load(f)]
            )
        )
    max_data["emoji"] = {}
